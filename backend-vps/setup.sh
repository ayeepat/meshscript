...
MODEL_ADMIN_KEY=
# Must exactly match the Worker secret set with:
#   npx wrangler secret put ENTITLEMENT_SECRET
# Use at least 32 random bytes (for example: openssl rand -hex 32).
ENTITLEMENT_SECRET=
# Base64 of the P-256 PEM used by scripts/sign-runtime-config.mjs. This lets
# dashboard feature switches publish the same signed policy to the extension.
RUNTIME_CONFIG_PRIVATE_KEY_B64=
...
# PROXY_PDF_MODEL=gemini-2.5-flash-lite
# PROXY_PDF_FALLBACK_MODELS=gemini-2.5-flash
ENV_EOF
fi
...
${DOMAIN} {
    # Caddy access logging is intentionally not enabled: request URLs may
    # contain job identifiers and must not be retained.
