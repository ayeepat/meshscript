...
//
// IMPORTANT: this only emits a client effort hint, and on МЭШ work the VPS
// deliberately OVERRIDES it upward. The live model (qwen3.8-flash) does have
// real effort levels, but homework and tests always run at its deepest one —
// an "easy" misfire must never under-think a graded answer. The hint is only
// honoured on the cheap any-site chain, which sends tier:'standard' with it.
// See the effort comments in qwen.js / deepseek.js / smesh-proxy.js and the
// per-model policy in backend-vps/server.js.
//
