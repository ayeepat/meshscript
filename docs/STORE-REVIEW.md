...
| `https://smeshapi.site/*` | License check. A single `GET /verify?key=…&device_id=…` to our own Cloudflare Worker. Sends only the license key the user typed and an anonymous random device UUID — no homework, no Mesh data, no personal info. |
